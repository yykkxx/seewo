...
**用于关闭希沃易启学（易课堂）学生端管控的 Windows 工具 · v4.4 · 双进程架构**

> 统一品牌名 `seewokiller`：窗口标题（初始）、托盘、互斥锁、数据目录、日志/状态文件、防火墙规则前缀全部使用同一命名。
...
挂起/恢复前会自动启用 `SeDebugPrivilege`（非管理员时提示需要管理员权限）。与「杀进程」的区别：**挂起不结束进程**。进程仍留在任务管理器里，但所有线程停止执行。对方的看护（`classroom-protect.exe`）通常按「进程是否还在」判断存活，因此挂起**不会触发重启**，比持续杀进程省得多，也不需要周期轮询。

代价是：目标窗口会变成无响应状态。**退出保护**：完全退出时自动恢复线程并拉起希沃；异常退出（进程被强杀）时由守护进程依据状态文件恢复线程、拉起希沃并清理防火墙。
...
| 功能        | 实现                                             | 说明                                                                                               |
| --------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **取消置顶**  | `window_ops.set_zbid_and_notopmost`            | 对目标 exe 的**全部窗口**逐一执行：`SetWindowBand` 降出高 ZBID + 「先 `HWND_TOPMOST` 再 `HWND_NOTOPMOST`」抖动强制刷新 Z 序 |
...
`window_ops.block_network` 用 `netsh advfirewall` 给每个目标进程加一条**出站阻止**规则，规则名为 `seewokiller_Block_<文件名>`。**每条规则 add 之后都会用 `show rule` 复核**，只有确认「规则存在且 Action=Block」才计入成功数，netsh 静默失败不会误报；「允许网络」删除后同样复核确认。

> ⚠️ 这些防火墙规则**持久化保存在系统中**。程序已做三层自动清理：① 正常退出时无条件清理；② 守护进程在 GUI 异常退出（看护放弃/守护退出）时清理；③ 守护进程下次启动时清理上次残留。强杀到两个进程都不在的极端情况，下次运行会自动补清。
...
- **窗口标题随机化**：维持自身置顶的同时，按 `config.TITLE_RANDOMIZE_SECONDS`（v4.2 起默认 **2 秒**，v4.1 是 10 秒）把标题换成 6–13 位随机字母数字，避免被按窗口标题枚举。界面初始显示名为 `seewokiller`（`config.BRAND`）。
...
点「完全退出」时，GUI **不会立刻结束自己**，而是先恢复现场、再按序退出：

1. 若目标线程被挂起，先恢复线程；再拉起希沃主程序窗口；最后无条件清理防火墙规则；
2. 经 IPC 发出 `shutdown` 指令；
...
| 项         | v4.3                  | v4.4                                              |
| -------- | --------------------- | ------------------------------------------------- |
| 命名统一     | `往昔的涟漪` / `SeewoGuard` 混用 | 全部统一为 `seewokiller`（`BRAND` 一处配置）                   |
| 防火墙规则    | 无验证、退出不清理            | `show rule` 复核生效；退出 / 守护放弃 / 守护启动三层自动清理             |
| 取消置顶     | 只处理第一个窗口             | 处理目标 exe 全部窗口                                  |
| 退出行为     | 挂起状态仅警告              | 自动恢复线程 + 拉起希沃 + 清理防火墙                           |
| 进程树对抗    | GUI/守护互为子进程，杀树一锅端    | 孤儿化脱离式互拉（中间进程派生），`taskkill /T` 杀不了一锅端             |
| 守护看护     | 5 次失败即放弃              | 15 分钟时间窗 10 次预算 + 重拉前探测 GUI 存活 + 8s 拉起冷却（防多开）      |
| 挂起权限     | 依赖启动时提权              | 挂起 / 恢复前自动启用 SeDebugPrivilege                    |
...
| 日志与状态    | 写在 exe 所在目录    | 迁到 `%LOCALAPPDATA%\seewokiller\`             |
...
2. **防火墙规则已自动清理** —— 正常退出、守护进程放弃看护、守护进程下次启动时都会清理残留规则；只有「两个进程同时被强杀」这一种极端情况需要下次运行补清。
3. **防火墙规则已验证生效** —— 每条规则 add 后用 `show rule` 复核，只有确认存在且为 Block 才计数；删除同样复核。
...
11. **命名已统一** —— 全部对外痕迹统一为 `seewokiller`（窗口标题/托盘/互斥锁/数据目录/日志/状态文件/防火墙规则前缀），`BRAND` 一处配置全局生效。
...
> **关于抗结束的边界**：本工具通过「双进程互相看护 + 脱离进程树拉起」实现对抗任务管理器/`taskkill /T` 的结束（被杀的进程会被另一进程自动拉回），并不拦截 `TerminateProcess` 本身，也不使用关键进程标记（避免蓝屏风险）。
